...
function updateGameArea() {
  ...
  if (gameArea.time >= LEVEL_COMPLETION_TIME) {
    ...
    if (currentLevel === totalLevels) gameComplete();
      ...
      var levelTransitionModalContent = document.getElementById('levelTransitionModalContent');
      levelTransitionModalContent.innerHTML += `<p id="coinMessage" class="levelTransitionMessage">Деньги 💲: ${currentCoins}</p>`;
      levelTransitionModalContent.innerHTML += `<p id="pointsMessage" class="levelTransitionMessage">Очки 💀: ${currentScore}</p>`;
    }
  }
